...
void NDotL_half(half3 Normal, half3 LightDirection, half ShadingOffset, half ShadingWidth, half ShadingFactor,
                half Flatness, out half Shading) {
    const half nDotL = dot(Normal, LightDirection) * 0.5 + 0.5;
    const half angleDiff = saturate(nDotL * ShadingFactor - ShadingOffset);
    const half angleDiffTransition = smoothstep(0, ShadingWidth, angleDiff);
    Shading = lerp(angleDiff, angleDiffTransition, Flatness);
}
