...
half4 StylizedPassFragment(Varyings input) : SV_Target
{
    ...
    half4 diffuse = SampleAlbedoAlpha(input.uv, TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap));

    const half alpha = diffuse.a;
    ...
    const half3 emission = half3(0, 0, 0); //SampleEmission(input.uv, _EmissionColor.rgb, TEXTURE2D_ARGS(_EmissionMap, sampler_EmissionMap));

    InputData inputData;
    InitializeInputData(input, normalTS, inputData);

    // Computes direct light contribution.
    half4 color = UniversalFragment_DSTRM(inputData, diffuse.rgb, emission, alpha);

    {
#if defined(_TEXTUREBLENDINGMODE_ADD)
        color.rgb += lerp(half3(0.0f, 0.0f, 0.0f), diffuse.rgb, _TextureImpact);
#else  // _TEXTUREBLENDINGMODE_MULTIPLY
        color.rgb *= lerp(half3(1.0f, 1.0f, 1.0f), diffuse.rgb, _TextureImpact);
#endif
    }
